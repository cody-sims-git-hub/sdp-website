...
- **Contact form posts to the SDP platform backend.** `app/components/contact-form.tsx` POSTs JSON to `platform.simsdigitalpartners.com` — the **`sdp-platform`** Laravel control-plane backend on the VPS. Its `POST /contact` endpoint resolves the client by the `Origin` header, enforces that client's CORS allow-list, and stores the submission in the client's own database (throttled). Protected by Cloudflare Turnstile plus a honeypot. It uses `app/lib/contact-api.ts` (typed client), `app/lib/contact-config.ts` (reads `VITE_CONTACT_API_URL` + `VITE_TURNSTILE_SITE_KEY`, both public, baked in at build), and `app/components/turnstile.tsx` (the widget). On success it shows an inline confirmation; on failure it falls back to a `mailto:` link. The page's other `mailto:` links (e.g. "Reach us directly") remain. **Adding or changing a form field means updating BOTH the platform (the `submissions` table + request validation in `sdp-platform`) and the frontend client + form.** The build needs `VITE_CONTACT_API_URL` and `VITE_TURNSTILE_SITE_KEY` set (see `.env.example`); these are public values, not secrets. (The retired `sdp-api` Node/Hono service served this before the 2026-06-28 cutover to `sdp-platform`.)
