...
- **Contact form posts to an API.** `app/components/contact-form.tsx` POSTs JSON to `api.simsdigitalpartners.com` (a separate **private** repo, `sdp-api`: Node/Hono + MySQL + Resend on the VPS), protected by Cloudflare Turnstile plus a honeypot. It uses `app/lib/contact-api.ts` (typed client), `app/lib/contact-config.ts` (reads `VITE_CONTACT_API_URL` + `VITE_TURNSTILE_SITE_KEY`, both public, baked in at build), and `app/components/turnstile.tsx` (the widget). On success it shows an inline confirmation; on failure it falls back to a `mailto:` link. The page's other `mailto:` links (e.g. "Reach us directly") remain. **Adding or changing a form field means updating BOTH the API (validation schema + DB column) and the frontend client + form.** The build needs `VITE_CONTACT_API_URL` and `VITE_TURNSTILE_SITE_KEY` set (see `.env.example`); these are public values, not secrets.
